...
- **hasMany and belongsTo Relationship Helpers**: Clearer relationship semantics with automatic record count calculation
  - New `t.hasMany()` method for defining one-to-many relationships
  - New `t.belongsTo()` method for defining many-to-one relationships
  - Automatic child table record count calculation based on `hasMany` min/max constraints
  - Virtual relationship fields (don't appear in generated output)
  - Full validation support for relationship constraints
  - Updated e-commerce and blog examples to demonstrate new features

### Changed

- Enhanced generator to skip virtual relationship fields in output
- Updated validator to check hasMany/belongsTo constraints and detect circular dependencies
- Improved documentation in README with dedicated relationship section
...
[0.4.0]: https://github.com/calvin-kimani/dev-db/compare/v0.3.0...v0.4.0
